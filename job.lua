		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction',
			{
				...
				elements	= {
					...
					{label = _U('hospital_menu'), value = 'sendto_hostiptal'},
					{label = _U('ems_menu_putincar'), value = 'put_in_vehicle'},
					{label = 'Repair car', value = 'repair_car'}
				}
			}, function(data, menu)
				...
				if closestPlayer == -1 or closestDistance > 3.0 then
					...
					elseif data.current.value == 'repair_car' then
						local elements  = {}
						local playerPed = PlayerPedId()
...
function OpenHospitalMenu()
  ...
  ESX.UI.Menu.Open(
    'default', GetCurrentResourceName(), 'hospital',
    {
      title    = _U('hospital_menu'),
      align    = 'top-left',
      elements = {
        {label = _U('patient_brokenarm'), value = 'brokenarm'},
        {label = _U('patient_headinjury'), value = 'headinjury'},
        {label = _U('patient_dead'), value = 'dead'},
      },
    },
    function(data, menu)
      menu.close()
      ...
      if closestPlayer == -1 or closestDistance > 3.0 then
        ...
        if data.current.value == 'brokenarm' then
          --TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(closestPlayer))
          TriggerServerEvent('HOSPITAL:hospitalize', GetPlayerServerId(closestPlayer), '60', 'broken arm', 0)
        end
        if data.current.value == 'shotwound' then
          --TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(closestPlayer))
          TriggerServerEvent('HOSPITAL:hospitalize', GetPlayerServerId(closestPlayer), '120', 'Gunshot wound', 0)
        end
        if data.current.value == 'headinjury' then
          --TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(closestPlayer))
          TriggerServerEvent('HOSPITAL:hospitalize', GetPlayerServerId(closestPlayer), '180', 'Head injury', 0)
        end
        if data.current.value == 'dead' then
          --TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(closestPlayer))
          TriggerServerEvent('HOSPITAL:hospitalize', GetPlayerServerId(closestPlayer), '300', 'no pulse possible death', 0)
        end
      end
    end,
    function(data, menu)
